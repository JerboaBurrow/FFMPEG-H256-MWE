...
void ffmpeg()
{
    ...
    std::vector<uint8_t> frame(width * height * 3, 0);
    ...
    for (int f = 0; f < 1*60; f++)
    {
        ...
        for (uint16_t i = 0; i < height; i++)
        {
            ...
            for (uint16_t j = 0; j < width; j++)
            {
                ...
                ray /= glm::length(ray);
                frame[i*width*3+j*3] = 5;
                frame[i*width*3+j*3+1] = 5;
                frame[i*width*3+j*3+2] = 5;
                for (auto & atom : atoms)
                {
                    ...
                    if (posnorm.first)
                    {
                        ...
                        frame[i*width*3+j*3] = uint8_t(255*colour.r*lighting);
                        frame[i*width*3+j*3+1] = uint8_t(255*colour.g*lighting);
                        frame[i*width*3+j*3+2] = uint8_t(255*colour.b*lighting);
                        break;
                    }
                }
                pj += dj;
...
void jo_mpeg()
{
    ...
    for (int f = 0; f < 1*60; f++)
    {
        ...
        jo_write_mpeg(fp, frame.data(), width, height, 60);
    }
    fclose(fp);
}
...
{
    bool jo = false;
    if (argc > 1) { if (std::string(argv[1]) == std::string("jo")) { jo = true; }}
    if (jo) { jo_mpeg(); }
    else { ffmpeg(); }
}